...
export default function HeroSection() {
  // Array of column opacity steps to replicate the stripe background effect
  const backgroundStripes = Array.from({ length: 24 });

  return (
    <section className="relative overflow-hidden bg-slate-50 text-slate-900 pt-16 pb-32 px-6 lg:px-12 border-b border-slate-200">
      
      {/* Background Vertical Stripe Pattern Layer */}
      <div className="absolute inset-0 grid grid-cols-12 sm:grid-cols-24 pointer-events-none z-0 opacity-40">
        {backgroundStripes.map((_, i) => (
          <motion.div
            key={i}
            initial={{ opacity: 0.2 }}
            ...
            className="h-full border-r border-blue-200/60 bg-gradient-to-b from-blue-50/80 via-blue-100/40 to-transparent"
          />
        ))}
      </div>
      ...
      <div className="max-w-7xl mx-auto flex flex-col items-center relative z-10">
        ...
        <motion.div 
          ...
          className="inline-flex items-center gap-2 bg-white text-[#2997D0] px-4 py-2 rounded-full text-xs font-semibold uppercase tracking-widest border border-blue-200 shadow-sm mb-6 backdrop-blur-md"
        >
        ...
        <div className="w-full relative grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
          ...
          >
            <div className="bg-white/90 backdrop-blur-xl p-6 rounded-3xl border border-slate-200 shadow-xl space-y-4">
          ...
          <motion.div 
            ...
            className="lg:col-span-6 bg-white/95 backdrop-blur-2xl rounded-3xl border border-slate-200/90 shadow-[0_20px_50px_rgba(0,0,0,0.08)] overflow-hidden"
          >
